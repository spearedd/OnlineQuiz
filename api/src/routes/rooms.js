...
router.use(
  ...
  catchErrors(async (req, res, next) => {
    req.room = await Room.findOne({ code: req.params.roomCode });
...
router.patch(
  ...
  catchErrors(async (req, res) => {
    ...
    if (roomClosed !== undefined) {
      if (req.room.teams.length < 1 || req.room.teams.length > 10) {
...
router.delete(
  ...
  catchErrors(async (req, res) => {
    req.room.ended = true;
    await req.room.save();
    res.json({ message: req.room.roomCode + ' Room has been ended.' });
  })
);
//#endregion
...
router.post(
  ...
  catchErrors(async (req, res) => {
    ...
    res.json({ message: req.team + ' Team has been approved.' });
  })
);
...
router.patch(
  ...
  catchErrors(async (req, res) => {
    ...
    switch (req.session.role) {
      ...
      default:
        return res.status(400).json({ message: 'Player is not allowed to perform this action.' });
    }
  })
);
...
router.post(
  ...
  catchErrors(async (req, res) => {
    ;
    console.log("Posting under role : " + req.session.role)
    req.session.role = SCOREBOARD;
    req.session.roomID = req.room._id;
    console.log("Role changed : " + req.session.role)
    req.room.scoreboards.push(req.sessionID);
    await req.room.save();

    req.session.save(() => res.json({ message: 'You are viewing scoreboard for : ' + req.session.roomID }));
  })
);
//#endregion
